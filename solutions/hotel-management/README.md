...
Search room availability for a date range, reserve and pay, check guests in and out, and maintain charges without overlapping bookings.
...
- Is inventory per room or room type?
- Are checkout dates exclusive?
- How long is a pending hold valid?
- Which cancellation/no-show policy applies?
- How are maintenance and housekeeping represented?

### Final Requirements

1. Search rooms by dates and capacity.
2. Quote and hold selected rooms.
3. Confirm after payment and allow eligible cancellation.
4. Check in only when dates and room state permit.
5. Add folio charges and complete checkout.
...
|---|---|
| Room | Owns physical room and operational state. |
| Hotel | Owns room inventory and availability queries. |
| Booking | Owns guest, date range, assigned rooms, and lifecycle. |
| RoomQuote | Freezes priced stay information. |
| Charge | Records one folio item. |
| PricingStrategy | Calculates nightly price. |
| BookingService | Coordinates reserve, pay, check-in, and checkout. |
...
Separate operational room state from future date-range availability and use half-open stays.

### Great Solution

Recheck and claim inventory atomically, model hold expiry and payment idempotency, and preserve quote/folio snapshots.

### Final Class Design

The critical collaboration is: search/quote -> atomically hold room range -> pay/confirm -> check in -> add charges -> settle/checkout -> release operational occupancy.
...
Implement one vertical slice before filling every class:
    ...
    search/quote -> atomically hold room range -> pay/confirm -> check in -> add charges -> settle/checkout -> release operational occupancy
...
- Room-type inventory and overbooking
- Housekeeping, maintenance, and no-shows
- Corporate rates and multi-room reservations
...
Explain date-range constraints, hold expiry, payment failure, concurrent booking, and the difference between physical and sellable inventory.

## Interview Walkthrough

1. Clarify the version-one scope and exclusions.
2. State the invariants before drawing classes.
3. Introduce the core entities and walk: search/quote -> atomically hold room range -> pay/confirm -> check in -> add charges -> settle/checkout -> release operational occupancy.
...
Or from inside `solutions/hotel-management`:
...
explicit eventual-consistency behaviorÃ¢â‚¬â€not merely additional classes.
...
and failure behaviorÃ¢â‚¬â€not around memorizing class diagrams.
